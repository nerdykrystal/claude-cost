...
description: Reusable template for authoring a TQVCD (Test Quality + Verification Coverage Document) as a prerequisite input to /dare-to-rise-code-plan. Defines what success looks like verification-wise — the verification-coverage headline metric (behaviors-verified / behaviors-claimed), applicable Testing Taxonomy categories with exit criteria, applicable standards operationalized as exit criteria, applicable benchmarks with target scores, coverage floors (internal CI metric only — NOT user-facing), accessibility criteria (WCAG 2.1 AA legal floor PLUS the LIVED floor — cognitive ADHD-conscious design / reading dyslexia-conscious typography / vision user-controlled theme toggle, per v06 expansion), performance budgets, two-state traceability tables linking FRs to test paths, and the headline-metric ban list for user-facing copy.
type: template
skill: dare-to-rise-code-plan
version: v06_I
date: 2026-05-05
methodology_version: 0.4.0
supersedes: TQVCD_Template_2026-04-27_v05_I.md (moved to references/deprecated/ on 2026-05-05 per D2R Accessibility Floor Update — §6 expansion of the legal-floor-only baseline to include the LIVED floor: cognitive ADHD-conscious design + reading dyslexia-conscious typography + vision user-controlled theme toggle. v05 superseded TQVCD_Template_2026-04-27_v04_I.md per Mod 8 production_pattern field addition; v04 superseded TQCD_Template_2026-04-26_v03_I.md per Mod 6.5 TQCD→TQVCD rename.)
v06_changes: §6 Accessibility Criteria expanded from "WCAG 2.1 AA hardwired" baseline to the FULL LIVED FLOOR. WCAG 2.1 AA continues as the legal/structural baseline (§6.1). New §6.5 Cognitive Accessibility (ADHD-conscious design) — zen-focus default, no unsolicited modal interruption, cognitive-load-aware defaults, friction-at-entry minimization. New §6.6 Reading Accessibility (dyslexia-conscious typography) — at-least-one research-backed dyslexic-friendly font (Lexend / OpenDyslexic / Atkinson Hyperlegible candidate set), tunable letter-spacing/line-height/paragraph-width, code surfaces excluded from font swap. New §6.7 Vision Accessibility (theme toggle MANDATORY) — dark/light user-controlled toggle, both themes WCAG 2.1 AA contrast, theme persistence, no surprise re-themes. New §6.8 TQVCD test-category mandatory entries cross-cut — visual-rendering (both themes pass contrast); interaction (theme-toggle correctness, modal-firing discipline, font-option swap); accessibility-cross-cut (dyslexic-font present + persists; theme persists; both themes pass aXe). §6.9 codifies the refusal table at gate (TQVCD that tests one theme but not both → REFUSE CRITICAL; TQVCD with only WCAG 2.1 AA scope → REFUSE HIGH; etc.). Empirical motivation: the lived floor is what daily-driver-instrument apps need for users with real accessibility profiles (neurodivergent / dyslexic / low-vision / variable-light-environments) to ADOPT and SUSTAIN use, not merely to legally access. Apps shipping with only the legal floor are MVP-deployable-failed regardless of WCAG compliance because they fail at scale when the lived floor is missing — users either don't adopt, adopt-and-abandon, or adopt-and-suffer. Per the larger principle codified in this update: accessibility floor = legal floor + lived floor. Methodology applies to ALL D2R apps, not Krystal-specific.
---
...
- Applies: [yes — always for UI, hardwired at the LIVED floor per §6 v06 expansion (legal floor WCAG 2.1 AA + cognitive + reading + vision)]
- Exit criteria: [WCAG 2.1 AA minimum (§6.1); cognitive accessibility tests (§6.5 + §6.8 modal-firing-discipline + chrome-quietness review); reading accessibility tests (§6.6 + §6.8 dyslexic-font option present + persists + swappable); vision accessibility tests (§6.7 + §6.8 theme-toggle correctness + both themes pass aXe + theme persistence); specific AAA items if required; automated tool results + manual testing protocol from TRD §3.5]
- Skip reason: [only valid if product has no UI at all]

**Snapshot / Visual Regression Tests** — `### TQVCD-TC-15`
- Applies: [yes/no — STRONGLY RECOMMENDED YES if §6.7 theme toggle present, since visual regression tests must run in BOTH themes per §6.8]
- Exit criteria if yes:
- Skip reason if no:
...
> - TQVCD-TC-14 (Accessibility) exit criteria address the LIVED floor (§6.5 cognitive + §6.6 reading + §6.7 vision), not just WCAG 2.1 AA legal floor (§6.1) — per v06 floor expansion
...
- *`### TQVCD-EC-02: WCAG 1.4.3 Contrast (Minimum)` — all text meets 4.5:1 (3:1 for large text). Verified by axe-core + manual spot check. Pass: zero violations at AA level IN BOTH THEMES per §6.7 + §6.8 (single-theme passing is not passing for apps with theme toggle).*
...
**§5.0 traceability table format (v05+ schema; production_pattern field required per Mod 8):**

| entry_id | behavior_claim | source_doc_ref | mutation_killing_test | last_passing_run | tautology_check | production_pattern |
|---|---|---|---|---|---|---|
| TQVCD-VC-01 | "Search returns within 250ms at p95" | TRD §3.1 NFR-3.1-01 | tests/perf/search-p95.k6.js | 2026-04-27T18:42:00Z | not-tautological (mutating threshold to 1ms fails the test) | PAT-LATENCY-INJECTION-UPSTREAM-TIMEOUT |
| TQVCD-VC-02 | "AES-256 encryption at rest" | SECURITY.md L42 | tests/security/encryption-at-rest.test.ts:18 | 2026-04-27T18:43:00Z | not-tautological (mutating bundled-sqlcipher → bundled fails the test) | PAT-GENERIC-NO-PRODUCTION-SHAPE (test verifies build-config integrity, not runtime production pattern) |
| TQVCD-VC-03 | "Works offline" | README.md L24 | tests/integration/offline-mode.spec.ts:7 | 2026-04-27T18:44:00Z | not-tautological (mutating fetch path to required-online fails the test) | PAT-FAILURE-INJECTION-DOWNSTREAM-PARTIAL-FAILURE (offline = downstream complete failure simulation) |

**`production_pattern` field schema (v05+, per Mod 8.1):**

Each TQVCD-VC entry MUST include `production_pattern` referencing either:

(a) **A canonical Production Pattern Catalog id** (preferred): `PAT-CONCURRENCY-LOCKFILE-SKIP`, `PAT-VOLUME-PAYLOAD-CROSSES-CACHE-TIER`, `PAT-SEQUENCE-OUT-OF-ORDER-DELIVERY`, `PAT-RESOURCE-PRESSURE-MEMORY-SATURATION`, `PAT-LATENCY-INJECTION-UPSTREAM-TIMEOUT`, `PAT-FAILURE-INJECTION-DOWNSTREAM-PARTIAL-FAILURE`, etc. See `_grand_repo/docs/Production_Pattern_Catalog_2026-04-27_v01_I.md` for the canonical list.

(b) **An inline `production_pattern_inline` block** (when no catalog pattern fits and the project hasn't yet promoted the pattern to catalog):

```yaml
production_pattern_inline:
  description: <production-shape description>
  input_load_sequence: <description of production conditions this VC's test applies>
  detection_signature: <what would signal this pattern is occurring in production>
  candidate_for_catalog_promotion: yes | no
```

(c) **A `PAT-GENERIC-NO-PRODUCTION-SHAPE` marker** (only when the behavior genuinely has no production-shape — e.g., build-config integrity tests, type-system tests, lint checks). Requires rationale in the field describing why no production pattern applies.

PSCAD (Pattern-Space Coverage Audit Document — see `PSCAD_Template_2026-04-27_v01_I.md`) audits TQVCD §5.0 from the pattern-space angle: PSCAD §4 coverage matrix entries reference TQVCD-VC entries by `entry_id`. Patterns referenced in §5.0 production_pattern field → pattern-space coverage. Patterns NOT covered by any TQVCD-VC → PSCAD §5 gaps with remediation plans.

> **Stop & Verify before continuing past §5.0.** Confirm:
> - Every user-facing claim from PRD / TRD / README / SECURITY / LAUNCH docs has a row in the §5.0 table with `entry_id` (TQVCD-VC-NN), `mutation_killing_test`, and `production_pattern` populated
...
> - Every row's `production_pattern` field is either a canonical catalog id OR an inline `production_pattern_inline` block OR a `PAT-GENERIC-NO-PRODUCTION-SHAPE` marker with rationale (per Mod 8.1 v05+ schema)
...
> - **PSCAD audit** (per /ideate-to-d2r-ready Phase 01 Step 01.6 + Phase 02 PSCAD↔TQVCD chain): every TQVCD-VC `production_pattern` reference appears in the corresponding PSCAD §3 patterns-in-scope OR has an honest-gap acknowledgment in PSCAD §5 if the pattern is not covered by any other TQVCD-VC
...
- `accessibility:axe-core` / `accessibility:keyboard` / `accessibility:screen-reader` — A11y gate per Layer in 6-layer chain (legal floor — WCAG 2.1 AA)
- `accessibility:cognitive` / `accessibility:reading` / `accessibility:vision-theme` — A11y gate per LIVED floor element (cognitive ADHD §6.5; reading dyslexia §6.6; vision theme-toggle §6.7) — added per v06 expansion
...
- **`fully accessible`** / **`WCAG-compliant`** / **`accessible`** (qualitative claim) without scoped disclosure — per v06 expansion: an app ships claiming "accessible" while only meeting the legal floor (WCAG 2.1 AA structural) is the lived-floor-omission failure mode. User-facing accessibility claim REQUIRES disclosure of which floor elements are met (legal §6.1 + cognitive §6.5 + reading §6.6 + vision §6.7) AND which are explicitly out of scope with rationale.
...
- **`meets WCAG 2.1 AA (legal floor) AND the D2R lived accessibility floor: cognitive (ADHD-conscious design) + reading (dyslexia-conscious typography) + vision (user-controlled theme toggle); see TQVCD §6 for scope`** — the v06-expanded accessibility scope, in user-facing form, with linked TQVCD §6 reference.
...
## 6. Accessibility Criteria — Legal Floor + Lived Floor `[requires TRD §3.5]`

**v06 expansion (load-bearing).** Prior to v06, this section codified WCAG 2.1 AA as the only accessibility floor. v06 expands the floor to include the LIVED floor — what users with real accessibility profiles need to USE the app as a daily-driver instrument, not merely to legally access it.

**The new floor (codified):**

- **§6.1 Legal floor — WCAG 2.1 AA (continues hardwired).** Structural accessibility: contrast ratios, semantic markup, keyboard nav, screen-reader labels.
- **§6.5 Lived floor — Cognitive accessibility (ADHD-conscious design).** New in v06.
- **§6.6 Lived floor — Reading accessibility (dyslexia-conscious typography).** New in v06.
- **§6.7 Lived floor — Vision accessibility (user-controlled theme toggle + contrast in both themes).** New in v06.
- **§6.8 Mandatory test-category cross-cut entries.** Operationalizes §6.5 + §6.6 + §6.7 into TQVCD test categories.
- **§6.9 Refusal table at gate.** Names what the floor REFUSES below.

The principle: **accessibility floor = legal floor + lived floor.** D2R apps that ship with only the legal floor are MVP-deployable-failed regardless of WCAG compliance, because daily-driver-instrument apps fail at scale when the lived floor is missing — users either don't adopt, adopt and abandon, or adopt and suffer.

### 6.1 Target Compliance — Legal Floor (WCAG 2.1 AA hardwired)
...
- Scope: every page, every component variant, every state, **AND every theme** (per §6.7 — automated accessibility test runs in EACH theme separately; single-theme passing is not passing)
...
- Color contrast spot-checking: dynamic-color elements verified IN BOTH THEMES
- Motion sensitivity: `prefers-reduced-motion` honored

### 6.4 Accessibility Exit Criteria For Every UI Stage (Legal Floor Specifics)

Author each as `### TQVCD-EC-Access-NN: ...`:

- Zero axe-core `critical` or `serious` violations IN BOTH THEMES (per §6.7)
- All functionality keyboard-operable
- All interactive elements have visible focus indicators
- All images have alt text or are marked decorative
- Color is never the sole indicator of meaning
- Contrast ratios meet 4.5:1 (normal text) and 3:1 (large text) minimums IN BOTH THEMES (per §6.7)
- Touch targets ≥ 44x44px for any mobile-targeted element
- `prefers-reduced-motion` respected on all animations

### 6.5 Cognitive Accessibility — ADHD-Conscious Design (LIVED FLOOR, NEW v06)

Apps where users do focused work (writing, reading, coding, planning) MUST satisfy:

- **`### TQVCD-EC-Access-Cog-01: Zen-focus default mode.`** App ships with minimal-chrome default; visible UI elements limited to those required for the immediate task; quiet visual environment (no animations or color changes that pull attention without user intent). Verified by UXD-anchored visual review against UXD §1.1 reference apps + UXD §1.3 polish criteria for chrome-quietness.
- **`### TQVCD-EC-Access-Cog-02: No unsolicited modal interruption.`** Modals fire on user action only, never on autoplay or scheduled timers (excluding security/safety-critical events documented in TRD). Verified by interaction test (modal-firing-discipline test): inject controlled timer events; assert no modal fires absent user action.
- **`### TQVCD-EC-Access-Cog-03: Cognitive-load-aware defaults.`** Settings ship with sensible defaults so the user does not have to make decisions before working. First-launch experience is "open the app and use it," not "configure 12 things first." Verified by first-launch-flow review (user opens app, can do primary task with zero configuration; any forced-onboarding step is a finding).
- **`### TQVCD-EC-Access-Cog-04: Friction-at-entry minimized.`** Time-to-first-productive-action measurable and bounded; specific bound declared in PRD §1.4 (non-visual excellence anchors). Verified by per-build performance test (time-to-first-productive-action, p95).

**Per-app cognitive-load specifics differ.** "Zen-focus default" for a markdown editor differs from "zen-focus default" for an accounting app. The floor mandates the discipline; per-app UXD specifies the manifestation. Honest gap: per-app interpretation is the responsibility of UXD §1.1 + §1.3 polish criteria; the floor mandates that EACH of TQVCD-EC-Access-Cog-01..04 has a populated test/review path, not that every app's test path is identical.

### 6.6 Reading Accessibility — Dyslexia-Conscious Typography (LIVED FLOOR, NEW v06)

Apps with rich-text, WYSIWYG, prose-rendering, or document-viewer surfaces MUST satisfy:

- **`### TQVCD-EC-Access-Read-01: Dyslexic-friendly typography option.`** At least one research-backed dyslexic-friendly font available as user-selectable option on rich-text/WYSIWYG content surfaces. Candidate set (per-app UXD authoring picks one or more):
  - **Lexend** — strongest reading-speed evidence (Renaissance Learning et al.); broad-spectrum reading improvement.
  - **OpenDyslexic** — most-known but mixed empirical evidence; strong opt-in user familiarity.
  - **Atkinson Hyperlegible** — BVI-anchored; works for some dyslexic readers; strong contrast properties.
  - All three are free / open-license; the floor mandates AT LEAST ONE available; per-app UXD picks; honest gap: the dyslexia-font research has live debate — don't relitigate the research in the floor.
- **`### TQVCD-EC-Access-Read-02: Tunable letter-spacing, line-height, paragraph-width.`** App provides reasonable defaults but allows user override on rich-text surfaces.
- **`### TQVCD-EC-Access-Read-03: Code surfaces excluded from font swap.`** Code blocks/inline-code retain monospace; dyslexic-friendly font applies to prose/rich-text only (rationale: code semantics depend on monospace alignment).
- **`### TQVCD-EC-Access-Read-04: Font option persists.`** User's font choice persists across sessions (verified by accessibility-cross-cut test: select dyslexic-font; reload; assert selection retained).

### 6.7 Vision Accessibility — Theme Toggle MANDATORY (LIVED FLOOR, NEW v06)

Apps with significant text content MUST satisfy:

- **`### TQVCD-EC-Access-Vis-01: Dark/light theme toggle is MANDATORY.`** User-controlled, not just system-follow. (System-follow may be ONE option but cannot be the only option.)
- **`### TQVCD-EC-Access-Vis-02: Both themes WCAG 2.1 AA contrast.`** Body text AND code (where applicable) hit AA contrast in BOTH themes. Single-theme passing is not passing. Verified by §6.2 automated tool run in EACH theme separately + manual contrast review for any custom-rendered text.
- **`### TQVCD-EC-Access-Vis-03: Theme persistence.`** User's theme choice persists across sessions.
- **`### TQVCD-EC-Access-Vis-04: No surprise re-themes.`** App does not auto-switch themes mid-session without explicit user action (excluding system-follow mode where the user has opted into following OS).

**Theme-toggle implementation is stack-dependent.** Tauri / Electron / SvelteKit each have idiomatic theme-switching mechanisms. The floor mandates the requirement; the sandbox-rules-* docs cover stack-specific implementation. Cross-reference, don't duplicate.

### 6.8 Mandatory Test-Category Cross-Cut Entries (operationalizes §6.5 + §6.6 + §6.7)

Per the existing D2R Verification-Coverage Principle (this template §5.0), each requirement above is testable; testable behavior must be tested. The floor expansion includes the test-discipline expansion:

- **`visual-rendering` test category** gains mandatory entries:
  - both themes pass automated contrast checks (§6.2 + §6.7)
- **`interaction` test category** gains mandatory entries:
  - theme-toggle correctness (§6.7 TQVCD-EC-Access-Vis-01..04)
  - modal-firing discipline (§6.5 TQVCD-EC-Access-Cog-02)
  - font-option swap correctness (§6.6 TQVCD-EC-Access-Read-01..04)
- **`accessibility-cross-cut` test category** gains mandatory entries:
  - dyslexic-font option present and persists (§6.6)
  - theme persists across sessions (§6.7)
  - both themes pass aXe equivalent (§6.7 + §6.2)

**Per-test rationale (TTB-discipline, per `_grand_repo/docs/test-tautology-bans.md`):** each new mandatory test must NOT be tautological per TTB-01..08; each must be one whose deletion would cause measurable system failure. Example: theme-toggle-correctness test mutating the theme-store-write-call to a no-op MUST fail the test (not-tautological); a test that asserts `themeToggle.exists` without asserting state-change behavior IS tautological and does NOT count.

### 6.9 Refusal Table At Gate (what the v06 floor REFUSES)

Per the floor expansion, the following conditions REFUSE at the TQVCD authorship gate AND at the cross-doc audit gate (`/ideate-to-d2r-ready` Phase 02):

| Condition | Severity | Rationale |
|---|---|---|
| TQVCD that tests one theme but not both | CRITICAL | Single-theme passing is not passing per §6.7; vision-floor violation; users in non-default theme suffer broken contrast / unreadable surfaces silently. |
| UXD that omits dyslexic-font option for WYSIWYG/rich-text apps | HIGH | Reading-floor violation per §6.6; dyslexic users cannot use the app's primary content surface comfortably. |
| App / PRD / TRD that assumes system-follow is sufficient substitute for user-controlled theme toggle | HIGH | Vision-floor violation per §6.7; users whose system theme doesn't match their needs (e.g., low-vision user on bright system default) have no recourse. |
| Stage 00 research that does not include accessibility-research track covering §6.5/§6.6/§6.7 (legal-floor-only research) | HIGH | Stage 00 outputs scope downstream Stage 01 plan; missing lived-floor research → missing lived-floor implementation. |
| TQVCD with only "WCAG 2.1 AA" as accessibility scope (no cognitive/reading/vision-specific tests) | HIGH | Codifies the lived-floor omission failure mode; the legal floor without the lived floor produces MVP-deployable-failed apps regardless of WCAG compliance. |
| Any of TQVCD-EC-Access-Cog-01..04 / Read-01..04 / Vis-01..04 marked NA without explicit rationale tied to product domain | MEDIUM (strict policy) | NA with rationale acknowledged is acceptable (e.g., a CLI tool with no rich-text surface NAs §6.6 with "no rich-text surface in product"); silent NA is the lived-floor-erosion failure mode. |

The /ideate-to-d2r-ready Phase 02 cross-doc audit consumes this refusal table at the same severity per the standards-alignment chain (TRD ↔ UXD ↔ TQVCD).
...
- [ ] §6 Accessibility floor satisfied at BOTH legal AND lived layers (per v06 expansion):
  ...
  - [ ] §6.5 Cognitive accessibility (TQVCD-EC-Access-Cog-01..04) — zen-focus / no-unsolicited-modal / cognitive-load-aware-defaults / friction-at-entry-minimized — each with test or review path OR NA-with-product-domain-rationale
  - [ ] §6.6 Reading accessibility (TQVCD-EC-Access-Read-01..04) — at-least-one dyslexic-friendly font / tunable type / code-surfaces-excluded / font-option-persists — each with test or NA-with-rationale (NA only valid when product has no rich-text/WYSIWYG/prose surface)
  - [ ] §6.7 Vision accessibility (TQVCD-EC-Access-Vis-01..04) — theme-toggle MANDATORY / both-themes-WCAG-AA / theme-persists / no-surprise-re-themes — each with test path; NA only valid when product has no significant text content
  - [ ] §6.8 Mandatory test-category cross-cut entries present in `visual-rendering` + `interaction` + `accessibility-cross-cut` categories per §6.5/§6.6/§6.7
  - [ ] §6.9 Refusal table conditions audited at this gate AND surfaced for /ideate-to-d2r-ready Phase 02 cross-doc audit
...
- [ ] `methodology_version: 0.4.0` declared in frontmatter
...
- **Stage QA:** the convergence loop executes the Testing Taxonomy categories declared applicable in §2.1 and §2.2, against the exit criteria declared throughout — INCLUDING §6.5/§6.6/§6.7/§6.8 lived-floor entries
...
**Cross-doc alignment chains:** TQVCD is the gate-operationalizer on EVERY chain leg; see `/ideate-to-d2r-ready` Phase 02 for the 11 chain audits including:
- Security chain ↔ §8
...
- Accessibility chain (legal + lived) ↔ §6.1-§6.9 (per v06 expansion: TRD §3.5 ↔ UXD §5 + UXD-cognitive/reading/vision specifics ↔ TQVCD §6.1-§6.9)
