...
Per `references/Heading_Prefix_ID_Grammar_2026-04-26_v01_I.md`, every load-bearing item appears as a heading with a strict ID prefix. TRD uses these TYPE prefixes:

- **FR** — Functional Requirement (§2.1)
- **BR** — Behavior Requirement (§2.2, §2.3)
- **NFR** — Non-Functional Requirement (§3, sub-typed by sub-section number, e.g., `NFR-3.1-02`)
- **INT** — Integration (§4)
- **DE** — Data Entity (§5.1)
- **TC** — Technical Constraint (§6)
- **AS** — Assumption (§7)
- **OQ** — Open Question (§9)

Example: `### TRD-FR-01: System MUST Generate D2R-Compliant Plans From 5-Doc Bundle`. Cross-doc references use the fully-qualified form: `references TRD-FR-01 + TRD-NFR-3.1-02`.

### Authorship Parallelization Markers

Section header markers indicate dependency for team-scaled authorship. Solo authors can ignore.
...
*What the system must do. Stated in testable terms. Author one heading per requirement (e.g., `### TRD-FR-01: System MUST Generate D2R-Compliant Plans`).*

*Required fields under each heading:*
- *Statement: "The system MUST [verb] [object] [under conditions]."*
- *Rationale: why this requirement exists (trace to PRD if possible — e.g., "implements PRD-UJ-02")*
...
*How the system behaves from the user's perspective. Author one heading per behavior (e.g., `### TRD-BR-01: When User Submits Bundle, System Confirms Receipt Within 250ms`). Required fields:*
- *Description*
- *Input/trigger*
...
*How the system behaves internally — background jobs, scheduled tasks, event processing, data transformations not directly user-visible. Author as `### TRD-BR-NN: ...` headings.*

---

## 3. Non-Functional Requirements `[requires §2]`

*Author one heading per NFR using the sub-section-aware ID format: `### TRD-NFR-{section}.{number}: {Title}`. Example: `### TRD-NFR-3.1-02: p95 Page Load Under 2.5s At 100 RPS`.*
...
> - All numbers are specific (p50/p95/p99 in milliseconds, throughput in RPS) not adjectives ("fast", "responsive")
> - Performance budget allocation totals 100% across tiers
> - Load profile assumption has a named source
...
*Feeds from Stage 00 Track 13 (Data Lifecycle & Privacy Engineering) AND Track 20 (Compliance, applicability-gated). For projects where Phase 1 Q12 declared Track 20 APPLICABLE, this section EXPANDS into a CRD-shaped block (jurisdiction × requirement matrix, per-rule traceability, data handling map, cross-border transfer mapping, subject-rights workflow, audit-log retention, compliance officer sign-off, last-audit-date).*
...
*If Track 20 APPLICABLE (per Phase 1 Q12), additionally include:*
- *Jurisdiction × requirement matrix (rows: jurisdictions; columns: applicable rules with version + section + effective date)*
- *Per-rule traceability (rule → product behavior triggering it → test verifying compliance → audit log evidencing compliance)*
- *Compliance-officer sign-off log (each major version, each material change)*
- *Last-audit-date + next-audit-due + audit-finding remediation log*

### 3.5 Accessibility Requirements

*WCAG 2.1 AA is hardwired — not an optional requirement to negotiate.*

*Required fields:*
- *Target compliance level (WCAG 2.1 AA minimum)*
- *Additional standards if any (AAA where relevant, Section 508, EN 301 549 for EU, ARIA Authoring Practices for specific widgets)*
...
*Feeds from Stage 00 Track 16 (Release Engineering & Versioning) outputs.*
...
*Feeds from Stage 00 Track 17 (Cost Modeling & FinOps) outputs. Applicability per PRD §6.5 + Phase 1 Q9.*

*Required fields (or "NA — [justification matching PRD §6.5]"):*
...
> **Stop & Verify before continuing past §3.10.** Confirm:
> - PRD §6.5 applicability decision matches this section's applicability
> - If APPLICABLE: budget ceiling is a specific currency amount
> - If NA: justification matches PRD §6.5 NA justification verbatim

### 3.11 Internationalization Requirements

*Feeds from Stage 00 Track 18 (Internationalization & Localization) outputs. Applicability per PRD §6.6 + Phase 1 Q10.*

*Required fields (or "NA — [justification matching PRD §6.6]"):*
...
*Every external service, API, or system the project integrates with. Author one heading per integration (e.g., `### TRD-INT-01: Anthropic API For LLM Inference`). Required fields:*
- *Integration purpose*
...
*Other internal systems or components this project integrates with. Author as `### TRD-INT-NN: ...` headings.*

### 4.3 Data Sources

*Every data source consumed. Author as `### TRD-INT-Source-NN: ...` headings. Required fields per source:*
- *Update frequency*
...
*Major data entities the system works with. Author one heading per entity (e.g., `### TRD-DE-01: D2R Plan`). Schema for each. Can reference external schema files.*
...
*Technologies that MUST be used. Author one heading per constraint (e.g., `### TRD-TC-01: Must Use Tauri For Desktop Surface`). Includes:*
...
*Technologies that MUST NOT be used, with reasons. Author as `### TRD-TC-NN: ...` headings.*
...
*Feeds from Stage 00 Track 12 (Deployment Architecture & Infrastructure-as-Code) outputs.*
...
*Author one heading per assumption (e.g., `### TRD-AS-01: Anthropic API Rate Limits Stable`). Required fields: why this is treated as given, what would invalidate it, fallback if invalidated.*
...
*Technical work explicitly deferred. Author as `### TRD-OS-NN: ...` headings. Each item has a rationale and, if applicable, a version/milestone for revisiting.*

---

## 9. Open Technical Questions `[independent]`

*Author one heading per question (`### TRD-OQ-01: Should We Use Server-Side Or Edge Functions?`). Stage 00 research should address these or the TRD must be updated with resolutions before Stage 01.*

---

## Amendment Protocol

*Per `references/Methodology_Versioning_And_Amendment_Protocol_2026-04-26_v01_I.md` §8 (canonical text). Authors of an instance TRD should paste the canonical Amendment Protocol section here. Summary: Phase A (authoring, pre-Stage-00) follows the inline validation hooks; Phase B (execution amendments) requires amendment-log entry + cross-doc audit re-run; Phase C (operational) requires amendment-log entry + ORD update if applicable.*

*Note: TRD §3 NFR amendments propagate to TQVCD §7 / §8 / §10 gates. Any post-Phase-A NFR amendment MUST trigger TQVCD re-audit.*
...
- [ ] Heading-prefix IDs assigned to all FRs / BRs / NFRs / integrations / data entities / constraints / assumptions / open questions
...
- [ ] §3.4 Privacy: applicable regulations declared (GDPR/CCPA/etc.) or NA justified; if Track 20 APPLICABLE, CRD-shaped block populated
...
- [ ] §3.10 Cost: budget ceiling + cost-control mechanisms specified, or NA-with-justification matching PRD §6.5
- [ ] §3.11 Internationalization: locale plan specified, or NA-with-justification matching PRD §6.6
...
- [ ] Inline validation hooks acknowledged at §3.1, §3.10
...
- [ ] Bundle Index sidecar reflects current TRD IDs (if CDCC v1.1.0 available)
...
This TRD is one of FIVE prerequisite inputs to `/dare-to-rise-code-plan`:
...
- **Track 1 — Tech Stack:** scopes against §2 (functional), §3 (non-functional), §4 (integration), §6.1/6.2 (mandatory/prohibited tech)
- **Track 2 — Applicable Standards:** scopes against §3.3 (security), §3.4 (privacy), §3.5 (accessibility), §3.1 (performance)
- **Track 3 — Applicable Benchmarks:** scopes against §3.1 (performance) and §3.6 (maintainability) targets
...
- **Track 17 — Cost Modeling & FinOps:** scopes against §3.10 (NA permitted with justification matching PRD §6.5)
- **Track 18 — Internationalization & Localization:** scopes against §3.11 (NA permitted with justification matching PRD §6.6)
- **Track 19 — AI / ML Engineering:** scopes against §2 + §3.3 (NA permitted when no AI in user-facing critical path; matches Phase 1 Q11)
- **Track 20 — Compliance & Regulatory:** scopes against §3.4 (expanded form) + PRD §6.2 (NA permitted when no regulated scope; matches Phase 1 Q12)

**Stage 01 onward:** The authored D2R code plan is backwards-planned from §2 (functional) + §3 (non-functional) + TQVCD acceptance criteria + Stage 00 research findings across all 16+4 tracks.

**Cross-doc alignment chains:** TRD is on EVERY chain leg; see `/ideate-to-d2r-ready` Phase 02 for the 11 chain audits.
