...
supersedes: TQVCD_Template_2026-04-27_v04_I.md (moved to references/deprecated/ on 2026-04-27 per Mod 8 production_pattern field addition; v04 superseded TQCD_Template_2026-04-26_v03_I.md per Mod 6.5 TQCD→TQVCD rename)
...
v05_changes: Per Methodology Mods Batch 2 Mod 8 (Production-Pattern Parity Testing + PSCAD as 6th D2R doc), the §5.0 verification-coverage traceability table gains a `production_pattern` field on every VC entry. Each TQVCD-VC entry now references either a canonical Production Pattern Catalog id (e.g., PAT-CONCURRENCY-LOCKFILE-SKIP) OR an inline `production_pattern_inline` block describing the production-shaped condition the test applies. PSCAD (Pattern-Space Coverage Audit Document) is the new 6th D2R sibling doc that audits TQVCD's coverage from the pattern-space angle; PSCAD §4 coverage matrix entries reference TQVCD-VC entries by ID. Together TQVCD §5.0 + PSCAD §4 close the failure mode where tests pass under tested load but deviate under production-shaped concurrency / volume / sequence patterns the test surface didn't apply (CDCC plugin Stage 05 proper-lockfile skip empirical case).
---
