...
description: Reusable template for authoring a TQCD as a prerequisite input to /dare-to-rise-code-plan. Defines what success looks like quality-wise — applicable Testing Taxonomy categories with exit criteria, applicable standards operationalized as exit criteria, applicable benchmarks with target scores, coverage floors, accessibility criteria, performance budgets, two-state traceability tables linking FRs to test paths.
type: template
...
Per `references/Heading_Prefix_ID_Grammar_2026-04-26_v01_I.md`, every load-bearing item appears as a heading with a strict ID prefix. TQCD uses these TYPE prefixes:
...
Example: `### TQCD-TC-04: End-to-End Tests`. Cross-doc references use the fully-qualified form: `references TQCD-EC-12 + TRD-NFR-3.1-02`.
...
For every category in the Taxonomy, this section declares applicability: YES (with exit criteria) or NO (with reason). Each declared category gets a heading with `### TQCD-TC-NN: ...` ID format.
...
**Unit Tests** — `### TQCD-TC-01`
- Applies: [yes/no]
- Exit criteria if yes: [coverage threshold, specific testing patterns expected]
- Skip reason if no:

**Integration Tests** — `### TQCD-TC-02`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Component / Module Tests** — `### TQCD-TC-03`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**End-to-End (E2E) Tests** — `### TQCD-TC-04`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Functional Tests** — `### TQCD-TC-05`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Regression Tests** — `### TQCD-TC-06`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Smoke Tests** — `### TQCD-TC-07`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Sanity Tests** — `### TQCD-TC-08`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Acceptance Tests (UAT)** — `### TQCD-TC-09`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Contract Tests (API-level)** — `### TQCD-TC-10`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Performance Tests** — `### TQCD-TC-11`
- Applies: [yes/no]
- Exit criteria if yes: [specific p50/p95/p99 targets from TRD]
- Skip reason if no:

**Security Tests** — `### TQCD-TC-12`
- Applies: [yes/no]
- Exit criteria if yes: [specific standards from TRD §3.3]
- Skip reason if no:

**Usability Tests** — `### TQCD-TC-13`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Accessibility Tests** — `### TQCD-TC-14`
...
**Snapshot / Visual Regression Tests** — `### TQCD-TC-15`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no:

**Property-Based / Fuzz Tests** — `### TQCD-TC-16`
- Applies: [yes/no]
- Exit criteria if yes: [specific invariants the property tests enforce]
- Skip reason if no:

**Mutation Testing** — `### TQCD-TC-17`
- Applies: [yes/no]
- Exit criteria if yes: [mutation score threshold]
- Skip reason if no:

**CI/CD Pipeline Tests** — `### TQCD-TC-18`
...
**Canary / A/B Testing (in production)** — `### TQCD-TC-19`
- Applies: [yes/no]
- Exit criteria if yes:
- Skip reason if no: [typically MVP has single deploy target; skip with this reason]

**Monitoring / Observability (post-deploy)** — `### TQCD-TC-20`
- Applies: [yes/no]
...
|---|----------|-----|---------|----------------------------------------|
| 1 | Classic Stress Test | TQCD-TC-S01 | | |
| 2 | Spike Testing | TQCD-TC-S02 | | |
| 3 | Breakpoint Testing | TQCD-TC-S03 | | |
| 4 | Soak (Endurance) Testing | TQCD-TC-S04 | | |
| 5 | Volume Testing | TQCD-TC-S05 | | |
| 6 | Scalability Testing | TQCD-TC-S06 | | |
| 7 | Capacity Testing | TQCD-TC-S07 | | |
| 8 | CPU Stress Testing | TQCD-TC-S08 | | |
| 9 | Memory Stress Testing | TQCD-TC-S09 | | |
| 10 | Disk I/O Stress Testing | TQCD-TC-S10 | | |
| 11 | Network Bandwidth Stress | TQCD-TC-S11 | | |
| 12 | File Descriptor / Handle Exhaustion | TQCD-TC-S12 | | |
| 13 | Service Dependency Failure | TQCD-TC-S13 | | |
| 14 | Cascading Failure Simulation | TQCD-TC-S14 | | |
| 15 | Partition Testing (Network Splits) | TQCD-TC-S15 | | |
| 16 | Retry Storm Testing | TQCD-TC-S16 | | |
| 17 | Queue Backlog Stress | TQCD-TC-S17 | | |
| 18 | Chaos Engineering Tests | TQCD-TC-S18 | | |
| 19 | Fault Injection Testing | TQCD-TC-S19 | | |
| 20 | Latency Injection | TQCD-TC-S20 | | |
| 21 | Resource Throttling | TQCD-TC-S21 | | |
| 22 | Database Contention Testing | TQCD-TC-S22 | | |
| 23 | Cache Stress Testing | TQCD-TC-S23 | | |
| 24 | Data Corruption Simulation | TQCD-TC-S24 | | |
| 25 | Large Payload Testing | TQCD-TC-S25 | | |
| 26 | Concurrent User Stress | TQCD-TC-S26 | | |
| 27 | Peak Traffic Pattern Simulation | TQCD-TC-S27 | | |
| 28 | Abusive / Edge Behavior Testing | TQCD-TC-S28 | | |
| 29 | DDoS Simulation | TQCD-TC-S29 | | |
| 30 | Authentication Flood Testing | TQCD-TC-S30 | | |
| 31 | UI Stress Testing | TQCD-TC-S31 | | |
| 32 | Device Resource Stress | TQCD-TC-S32 | | |
| 33 | Auto-Scaling Stress | TQCD-TC-S33 | | |
| 34 | Cold Start Stress (serverless) | TQCD-TC-S34 | | |
| 35 | Deployment Stress | TQCD-TC-S35 | | |
| 36 | Configuration Stress | TQCD-TC-S36 | | |
| 37 | Mixed Workload Testing | TQCD-TC-S37 | | |
| 38 | Game Day / Fire Drill Testing | TQCD-TC-S38 | | |
| 39 | Recovery Testing (under stress) | TQCD-TC-S39 | | |
...
*For each standard referenced in the TRD, author one heading per exit criterion (e.g., `### TQCD-EC-01: WCAG 2.1 AA — 1.1.1 Non-text Content`). Required fields:*
...
- *`### TQCD-EC-01: WCAG 1.1.1 Non-text Content` — all images have alt text or aria-label. Verified by axe-core. Pass: zero violations.*
- *`### TQCD-EC-02: WCAG 1.4.3 Contrast (Minimum)` — all text meets 4.5:1 (3:1 for large text). Verified by axe-core + manual spot check. Pass: zero violations at AA level.*

> **Stop & Verify before continuing past §3.1.** Confirm:
> - Every TRD-declared standard has at least one TQCD-EC entry
> - Every TQCD-EC has Verification method + Pass threshold (not "tested manually")

### 3.2 Regulatory Compliance Exit Criteria

*For each regulation applicable per TRD §3.4 (and Track 13 / Track 20), specific exit criteria that demonstrate compliance. Author as `### TQCD-EC-Reg-NN: ...` headings.*
...
*Author as `### TQCD-BG-NN: ...` headings. Required:*
- *Benchmark name*
- *Target score*
- *Measurement tool*
...
## 5. Coverage Floors `[requires TRD §2, PRD §4]`

### 5.1 Code Coverage

Per the D2R hardwired test coverage principle:

- **`### TQCD-CC-01: Line Coverage`** — 100% of backend + 100% of frontend testable code (exclude generated code, scaffolding, and third-party imports)
- **`### TQCD-CC-02: Branch Coverage`** — 100% of decision points
- **`### TQCD-CC-03: Integration Coverage`** — 100% of integration points between layers
...
Author each as `### TQCD-EC-Access-NN: ...`:
...
From TRD §3.1, §3.2, §3.10, §3.11, convert targets into measurable budgets and gates. Each budget gets `### TQCD-BG-NN: ...` ID.
...
> - Performance chain leg (TRD §3.1 ↔ AVD §4 ↔ TQCD §7.1) is COMPLETE
...
> - Cost chain (Phase 1 Q9 ↔ PRD §6.5 ↔ TRD §3.10 ↔ TQCD §7.5) shows COMPLETE legs OR matched-NA-justification at every leg
...
From TRD §3.3 Security Requirements (Tracks 9 + 15), operationalize as pre-commit and CI gates. Each gate gets `### TQCD-BG-Sec-NN: ...` ID.
...
Per D2R, every stage exit passes through an ASAE gate. This section declares ASAE Certainty Thresholds per stage type. Author each as `### TQCD-AT-NN: ...`:

- `### TQCD-AT-01: Stage 00 (research)` — threshold [default 2]
- `### TQCD-AT-02: Stage 01 (plan authorship)` — threshold [default 3]
- `### TQCD-AT-03: Stage 02-NN (implementation)` — threshold [default 3]
- `### TQCD-AT-04: Stage NN+1 (Design Polish)` — threshold [default 3, domain=design]
- `### TQCD-AT-05: Stage QA (convergence loop)` — threshold [default 5]
...
> - Observability chain (TRD §3.8 ↔ AVD-AC §3.1 Observability ↔ TQCD §10.1 ↔ runbook) shows COMPLETE legs
> - Release engineering chain (TRD §3.9 ↔ TQCD §10.2 ↔ runbook) shows COMPLETE legs
...
*Author one heading per question (`### TQCD-OQ-01: Should Mutation Score Threshold Be 80% Or 85%?`). Stage 00 research should address these or the TQCD must be updated before Stage 01.*

---

## Amendment Protocol

*Per `references/Methodology_Versioning_And_Amendment_Protocol_2026-04-26_v01_I.md` §8 (canonical text). Authors of an instance TQCD should paste the canonical Amendment Protocol section here. Summary: Phase A (authoring, pre-Stage-00) follows the inline validation hooks; Phase B (execution amendments) requires amendment-log entry + cross-doc audit re-run; Phase C (operational) requires amendment-log entry + ORD update if applicable.*

*Note: TQCD §5.2 traceability table updates are continuous in Phase B — every implementation stage commit adds `test_path` values. This is normal operating behavior, NOT an amendment-class change. Amendment-class changes to TQCD include: new test categories declared YES, new exit criteria, new ASAE thresholds, severity policy changes — these require amendment log entries.*
...
- [ ] Accessibility criteria specific (not "WCAG AA" — specific TQCD-EC-Access-NN entries + tools + protocols)
...
- [ ] ASAE thresholds declared per stage type (`TQCD-AT-NN`)
...
- [ ] Bundle Index sidecar reflects current TQCD IDs (if CDCC v1.1.0 available)
...
This TQCD is one of FIVE prerequisite inputs to `/dare-to-rise-code-plan`:
...
**Cross-doc alignment chains:** TQCD is the gate-operationalizer on EVERY chain leg; see `/ideate-to-d2r-ready` Phase 3 for the 11 chain audits including:
- Security chain ↔ §8
