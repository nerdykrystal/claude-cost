...
description: "Locks in the Clauda/Claudette W/L Value Genius role definition for the thread. TRIGGER PATTERN: claud*_*_value_genius (* = wildcard) - invoke for clauda_w/l (non-coding) and claudette_w/l (coding) _value_genius, or paraphrases naming 'Value Genius' with clauda/claudette + W/L. After invocation: persona for commit-trailer attribution computed from the matched trigger; canonical role-definition artifact loaded; constraints active. Use when: any thread invoking Value Genius for IP + market-value defense, experimental-portfolio articulation, methodology-recursion work, or transit-enabling deliverables (Anthropic warm-path outreach, paper publication, valuation defense, PEK regeneration, cold-assessment iteration, role-definition refinement). Applies whether the workstream is coding (Claudette) or non-coding (Clauda). Skip for: client work outside Martinez Methods; threads where Krystal directs a different role."
---
